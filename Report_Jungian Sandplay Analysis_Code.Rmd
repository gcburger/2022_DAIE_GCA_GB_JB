...
## Hypothesis
**Null hypothesis: therapy makes no difference, no matter the patient group (i.e. no difference between the pre and post_trial_cpss (oberserver-rated, therapist) scores for Animated and Control groups**

**Our Alternative hypothesis: The effect of animated VR therapy is more positive/sees better results than the effect of standard (no VR) therapy (i.e. the difference between pre and post_trial_cpss scores for the animated group is higher than that of the control group**

**could state that we are focusing on cpss scores and not observer rated scores with the assumption that they would be more accurate, or something like that**
...
study_data_filtered <- study_data %>%
  select(test_group, ends_with("cpss")) %>%
  filter(test_group %in% c("Control", "Animated") 
         & pre_trial_cpss > 0, pre_trial_cpss < 10
...
mean(study_data_filtered$pre_trial_cpss) # 5.9691
mean(study_data_filtered$post_trial_cpss) # 5.4046
...
pre_trial_cpss_mean <- mean(study_data_filtered$pre_trial_cpss)
pre_trial_cpss_std <- sd(study_data_filtered$pre_trial_cpss)

post_trial_cpss_mean <- mean(study_data_filtered$post_trial_cpss)
post_trial_cpss_std <- sd(study_data_filtered$post_trial_cpss)
```
...
```{r Histogram - pre_trial_cpss, echo=TRUE}
# draw histogram for pre_trial_cpss
hist(study_data_filtered$pre_trial_cpss, main="Pre Trial CPSS", xlab="Measured Score", ylab="Frequency", breaks=6, col='steelblue')
```

```{r Histogram - post_trial_cpss, echo=TRUE}
# draw histogram for post_trial_cpss
hist(study_data_filtered$post_trial_cpss, main="Post Trial CPSS", xlab="Measured Score", ylab="Frequency", breaks=6, col='steelblue')
```
...
qqnorm(study_data_filtered$pre_trial_cpss, pch = 2, frame = FALSE, main="Pre Trial CPSS")
qqline(study_data_filtered$pre_trial_cpss, col = "darkorchid", lwd = 1)
```

```{r QQ Plot - post_trial_cpss, echo=TRUE}
# perform qqnorm test on post_trial_cpss
qqnorm(study_data_filtered$post_trial_cpss, pch = 2, frame = FALSE, main="Post Trial CPSS")
qqline(study_data_filtered$post_trial_cpss, col = "darkorchid", lwd = 1)
```
...
pre_trial_cpss_sw <- shapiro.test(study_data_filtered$pre_trial_cpss)
pre_trial_cpss_sw
```
...
post_trial_cpss_sw <- shapiro.test(study_data_filtered$post_trial_cpss)
post_trial_cpss_sw
```
A `r post_trial_cpss_sw$method` was conducted on the post_trial_cpss data.\
From the output obtained we can assume normality as the p-value (p = `r round(post_trial_cpss_sw$p.value, 4)`) is greater than 0.05.
