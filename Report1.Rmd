...
study_data$impact <- as.dbl(study_data$test_group)
...
# Select variables to work with including adding variance column, then filter and arrange data (not removing >12 range yet)
  ...
  study_data %>%
    ...
    filter(
      ...
      (pre_trial_cpss > 0) &
      (post_trial_cpss < 10)) %>%
    mutate(impact_cpss = post_trial_cpss - pre_trial_cpss) %>%
    arrange(-impact_cpss) %>%
    View
    
    mean(study_data$impact_cpss)
    mean(study_data$impact_cpss[study_data$test_group=="Control"])
    mean(study_data$impact_cpss[study_data$test_group=="Static"])
    
## Hypothesis testing
    
    boxplot(test_group ~ impact)
    # Ho: mean impact of control group < those who completed therapy
    # two sided test
